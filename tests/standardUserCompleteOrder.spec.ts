...
test(
  ...
  async ({
    ...
    const item = await test.step('get product from items on the Inventory Page', async () => {
      return inventoryPage.getProductItem(productName);
    });
    const priceItem = await test.step('get product price for item from list', async () => {
    ...
    const cartItem = await test.step('get product items on the Cart Page  ', async () => {
      return cartPage.getProductCartItem(productName);
    });
    const cartItemPrice = await test.step('get product price from the Cart Page  ', async () => {
